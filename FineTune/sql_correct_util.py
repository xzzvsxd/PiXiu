        ...
        for row in result[:50]:
            ...
            for val in row:
                try:
                    ...
                    if "几家" in ori_question or "多少家" in ori_question:
                        vals.append('{:.0f}家'.format(num))
                    if "几个" in ori_question or "多少个" in ori_question:
                        vals.append('{:.0f}个'.format(num))
                    else:
                        vals.append('{:.2f}元'.format(num))
                except:
                    vals.append(val)
